...
const exactTextTranslations = {
  ...
  "A national-level undergraduate innovation project on intelligent TCM tongue diagnosis with D-FINE, SAM2, and feature-guided constitution analysis.": "一个基于 D-FINE、SAM2 与体征导向分析的中医智能舌诊国家级大学生创新项目。",
  "This project develops a multi-platform intelligent tongue diagnosis system for traditional Chinese medicine by integrating constitution theory with deep learning techniques. To address the practical limitations of traditional tongue inspection, including dependence on human experience as well as interference from lighting conditions and pose variations, the framework employs D-FINE for tongue detection, SAM2 for tongue-image segmentation, and an upright rectification algorithm for posture normalization. It further introduces a multi-branch, symptom-guided network with attention-based cross-region feature fusion for automatic tongue analysis. The system also constructs a standardized dataset of more than 5,109 tongue images collected under different lighting conditions across eight constitution types, providing a feasible path toward more objective and standardized tongue diagnosis while exploring its potential in mobile-assisted diagnostic scenarios.": "本项目围绕中医“望诊”中的舌诊环节，构建了一个融合中医体质理论与深度学习技术的多平台智能舌诊系统。针对传统舌诊依赖经验、易受光照条件与姿势角度等外部因素干扰的问题，框架采用 D-FINE 进行舌体检测、SAM2 进行舌象分割，并加入直立矫正算法实现图像姿态标准化。随后，系统设计了多分支体征导向网络，并通过注意力机制进行跨区域特征融合，以实现舌象图像的自动识别与处理。项目还构建了覆盖八种中医体质类型、包含 5109 余张不同光照条件舌象图像的标准化数据集，为中医望诊的客观化与标准化提供了可行路径，并探索了其在移动终端辅助诊断中的应用潜力。",
  "I led the design and development of the project's core deep learning algorithms. I independently implemented the three major modules of tongue detection, tongue segmentation, and constitution classification, and further designed an upright rectification algorithm to standardize tongue-image posture. I organized the dataset into training and test sets for model development and completed multi-label classification over eight tongue-related attributes. Finally, I established a five-fold cross-validation pipeline and conducted rigorous ablation studies using core metrics such as F1-score and Jaccard Index, which improved the robustness of diagnosis in complex scenarios.": "我主导了项目核心深度学习算法的设计与研发，独立完成了舌体检测、舌象分割和体质分类三大模块，并构建了直立矫正算法以实现图像姿态标准化。我将舌象数据划分为训练集与测试集用于模型训练和测试，并完成了 8 大舌部属性的多标签分类。最后，我建立了五折交叉验证体系，采用 F1-score、Jaccard Index 等核心指标进行了严格的消融实验分析，从而提升了复杂场景下的诊断鲁棒性。",
